...
async function quiz(){
 const body = `appid=apple-jd-aggregate&functionId=brandquiz_prod&body={"quizId":4,"quizStr":"${distinct(brandlistArr)}","predictId":null,"apiMapping":"/api/index/quiz"}&t=${new Date().getTime()}&loginType=2`
...
const body = `appid=apple-jd-aggregate&functionId=brandquiz_prod&body=%7B%22quizId%22:4,%22apiMapping%22:%22/api/support/getSupport%22%7D&t=${new Date().getTime()}&loginType=2`
...
async function getshareid(){
 const body = `appid=apple-jd-aggregate&functionId=brandquiz_prod&body={"quizId":4,"apiMapping":"/api/support/getSupport"}&t=${new Date().getTime()}&loginType=2`
 const MyRequest = PostRequests(body)
 return new Promise((resolve) => {
    $.post(MyRequest,async(error, response, data) =>{
    try{
...
async function getlist(){
 ...
 const body = `appid=apple-jd-aggregate&functionId=brandquiz_prod&body={"quizId":4,"apiMapping":"/api/index/brandRank"}&t=${new Date().getTime()}&loginType=2`
 const MyRequest = PostRequests(body)
 return new Promise((resolve) => {
    $.post(MyRequest,async(error, response, data) =>{
    try{
