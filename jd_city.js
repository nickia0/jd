...
!(async () => {
  ...
  inviteCodes = await getAuthorShareCode('https://raw.githubusercontent.com/Argon-av/backup/main/code/city.json')
  if (!inviteCodes) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Argon-av/backup/main/code/city.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    inviteCodes = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/Argon-av/backup/main/code/city.json')
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    cookie = cookiesArr[i];
    $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
